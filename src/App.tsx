      ...
      >
        ...
        {isTauri && (
          <button
            ...
            className="fixed top-4 right-4 z-[200] p-3 bg-black/20 hover:bg-black/40 text-white rounded-full opacity-0 group-hover/present:opacity-100 transition-opacity backdrop-blur-sm border border-white/10"
                ...
                >
                  ...
                  <div className={`${settings.oneVersePerLine || verse.isNewChapter || verse.acrostic ? 'flex gap-6 items-start w-full' : 'inline'}`}>
                    {settings.showVerseNumbers && (
                      <span 
                        className={`text-[0.6em] opacity-40 select-none mr-2 ${settings.oneVersePerLine || verse.isNewChapter || verse.isNewPassage || verse.acrostic ? 'mt-3 flex-shrink-0' : 'inline-block align-top mt-1'}`}
            ...
            >
              <div 
                ...
                style={{ 
                  ...
                  backgroundColor: settings.theme === 'dark' ? 'rgba(248, 250, 252, 0.95)' : 'rgba(15, 23, 42, 0.95)',
                  color: settings.theme === 'dark' ? '#0f172a' : '#f8fafc',
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
                ...
                {displayMode === 'dual' && (
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1 bg-amber-600/20 text-amber-500 border border-amber-500/30 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-amber-600/30 transition-all hover:scale-105"
                  >
                    <Tv size={12} /> Launch Display
                  </button>
                )}
              </div>
            ...
            <AnimatePresence>
              {!readingMode && showSettings && (
                ...
                >
                  ...
                  <div>
                    <label className={`text-[10px] font-bold uppercase tracking-widest ${uiTextMuted} mb-3 block`}>Display Surface</label>
                  ...
                  <div>
                    <label className={`text-[10px] font-bold uppercase tracking-widest ${uiTextMuted} mb-3 block`}>Typography</label>
                    <div className="grid grid-cols-2 gap-2">
                      {FONT_OPTIONS.map(font => (
                        <button 
                          key={font.id}
                          onClick={() => setSettings({...settings, fontFamily: font.id})}
                          className={`text-[10px] p-2 rounded border text-left transition-all ${settings.fontFamily === font.id ? 'bg-amber-500 text-slate-900 font-bold border-amber-600' : `${uiTheme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} ${uiTextMuted} hover:border-amber-500/50`}`}
                  ...
                  <div className={`space-y-4 border-t ${uiBorder} pt-4`}>
                    <h3 className={`text-[10px] font-bold uppercase tracking-widest ${uiTextMuted}`}>Default Bible</h3>
                    <select 
                      value={settings.defaultTranslation} 
                      onChange={(e) => {
                        const newTranslation = e.target.value;
                        setSettings({...settings, defaultTranslation: newTranslation});
                        setTranslation(newTranslation);
                        setVerses([]);
                        fetchBiblePassage(false, referenceInput, newTranslation);
                      }}
                  ...
                  <div className={`space-y-4 border-t ${uiBorder} pt-4`}>
                    <h3 className={`text-[10px] font-bold uppercase tracking-widest ${uiTextMuted}`}>Passage Layout</h3>
                    <div className="flex flex-col gap-3 grow">
                      <button 
                        onClick={() => setSettings({...settings, showVerseNumbers: !settings.showVerseNumbers})}
                        className={`flex items-center justify-between p-3 rounded-lg border text-xs transition-all ${settings.showVerseNumbers ? 'bg-amber-500/10 border-amber-500 text-amber-500 font-bold' : `${uiTheme === 'dark' ? 'bg-slate-800' : 'bg-white'} border-slate-200 ${uiTextMuted} hover:bg-amber-500/5`}`}
                      >
                        <span>Show Verse Numbers</span>
                        <div className={`w-8 h-4 rounded-full relative transition-colors ${settings.showVerseNumbers ? 'bg-amber-500' : 'bg-slate-200'}`}>
                      ...
                      <button 
                        ...
                        className={`flex items-center justify-between p-3 rounded-lg border text-xs transition-all ${settings.oneVersePerLine ? 'bg-amber-500/10 border-amber-500 text-amber-500 font-bold' : `${uiTheme === 'dark' ? 'bg-slate-800' : 'bg-white'} border-slate-200 ${uiTextMuted} hover:bg-amber-500/5`}`}
                      >
                        <span>One Verse Per Line</span>
                        <div className={`w-8 h-4 rounded-full relative transition-colors ${settings.oneVersePerLine ? 'bg-amber-500' : 'bg-slate-200'}`}>
                          <div className={`absolute top-1 w-2 h-2 rounded-full bg-white transition-all ${settings.oneVersePerLine ? 'right-1' : 'left-1'}`} />
                        </div>
                      </button>

                      <div className="flex flex-col gap-2 mt-2">
                        <label className={`text-[9px] font-bold uppercase tracking-widest ${uiTextMuted}`}>Verse Number Color</label>
                        <div className="flex gap-2">
                          <input 
                            ...
                            className={`w-full h-8 rounded cursor-pointer border ${uiBorder} p-0 overflow-hidden`}
                          />
                          <button 
                  ...
                  <div className={`space-y-4 border-t ${uiBorder} pt-4`}>
                    <h3 className={`text-[10px] font-bold uppercase tracking-widest ${uiTextMuted}`}>Page Settings</h3>
                    <div className={`${uiTheme === 'dark' ? 'bg-slate-800' : 'bg-white'} p-3 rounded-lg border ${uiBorder} shadow-sm ${uiText}`}>
                      <label className={`flex justify-between text-[10px] font-bold ${uiTextMuted} uppercase tracking-widest mb-2`}>
                  ...
                  <div className={`${settings.oneVersePerLine || verse.isNewChapter || verse.acrostic ? 'flex gap-4 items-start w-full' : 'inline'}`}>
                    {settings.showVerseNumbers && (
                      <span 
                        ...
                        className={`select-none opacity-50 font-bold text-[0.6em] align-top mt-[0.2em] inline-block shrink-0 ${isRtl ? 'ml-2' : 'mr-2'}`}
                      >
                        {verseNumber}
                      </span>
                    )}
